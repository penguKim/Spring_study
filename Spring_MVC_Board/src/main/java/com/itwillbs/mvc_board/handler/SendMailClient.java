...
public class SendMailClient {
	...
	public void sendMail(String email, String subject, String content) {
		try {
			...
			props.put("mail.smtp.port", "587"); // Gmail 서버 TLS 프로토콜 SMTP 포트 번호
			props.put("mail.smtp.auth", "true"); // SMTP 서버 접근 시 인증 여부 설정(로그인 필요 여부 설정)
			// 메일 서버 인증 관련 추가 정보 설정(설정 내용에 따라 위의 포트 번호가 바뀐다)
			...
			Address senderAddress = new InternetAddress("test@test.com", "아이티윌");
...
//		message.setHeader("content-type", "text/html; charset=UTF-8");
			...
			// ----------------------------------------------------
			// 7. 메일 전송
			//    javax.mail.Transport 클래스의 static 메서드인 send() 메서드 호출
