...
//		headers.add("Authorization", "Bearer " + map.get("access_token"));
	...
	public Map<String, Object> requestDeposit(Map<String, String> map) {
		...
		joReq.put("req_client_name", map.get("req_client_name")); // 요청고객성명(출금계좌 예금주명)
		joReq.put("req_client_fintech_use_num", map.get("fintech_use_num")); // 요청고객 핀테크이용번호(출금계좌)
		joReq.put("req_client_num", map.get("id").toUpperCase()); // 요청고객 회원번호(아이디 활용) => 단, 영문자는 모두 대문자 사용 
		...
		jo.put("req_list", jaReqList); // 1검의 입금이체 정보
...
//		headers.add("Authorization", "Bearer " + map.get("access_token"));
		...
		// ---------- 요청 고객(출금 대상) 정보 ----------
		jo.put("fintech_use_num", map.get("fintech_use_num")); // 출금계좌 핀테크이용번호
		jo.put("wd_print_content", map.get("recv_client_name")); // 출금계좌인자내역(이용기관에서 이용계약 시 설정하는 통장기재내용)
		jo.put("tran_amt", map.get("tran_amt")); // 거래금액
		jo.put("tran_dtime", bankValueGenerator.getBankTranDTime()); // 요청일시(자동생성)
		jo.put("req_client_name", map.get("req_client_name")); // 요청고객성명(출금계좌 예금주명)
		jo.put("req_client_fintech_use_num", map.get("fintech_use_num")); // 요청고객 핀테크이용번호(출금계좌)
		...
		// 이 정보(3개)는 피싱 등의 사고 발생 시 지급 정지를 위한 정보(현재 오픈뱅킹에서 검증수행은 X)
		// 이체용도 필드값이 송금(“TR”) 및 결제(“ST”)인 경우 이 값을 설정한다.
		jo.put("recv_client_name", map.get("recv_client_name")); // 상대방 예금주명
		jo.put("recv_client_bank_code", map.get("recv_client_bank_code")); // 상대방 은행코드
		jo.put("recv_client_account_num", map.get("recv_client_account_num")); // 상대방 계좌번호
	...
	public Map<String, Object> requestDeposit2(Map<String, String> map) {
		...
		joReq.put("tran_no", "1"); // 거래순번(단건이므로 무조건 1)
		joReq.put("bank_tran_id", bankValueGenerator.getBankTranId()); // 거래고유번호(자동생성)
		joReq.put("fintech_use_num", map.get("recv_client_fintech_use_num")); // 입금계좌 핀테크이용번호(상대방)
