...
// 자바 메일 기능 사용 시 메일 서버(ex. Gmail) 인증을 위한 정보 관리할 클래스 정의
// => javax.mail.Authenticator 클래스를 상속받는 서브클래스로 정의
public class GoogleMailAuthenticator extends Authenticator {
	// 인증정보(아이디, 패스워드(앱 비밀번호))를 관리할 javax.mail.PasswordAuthentication 타입 변수
	 ...
	 * - 파라미터 : 메일 서버 계정명, 패스워드
	 * => Gmail 기준 2단계 인증 미사용 시 : Gmail 계정명(도메인 생략), Gmail 계정 패스워드
	 * => Gmail 기준 2단계 인증 사용 시 : Gmail 계정명, 2단계 인증 우회 앱 비밀번호
	 * => 구글 계정 설정 - 보안 - 2단계 인증 - 앱 비밀번호 설정 필요
	 *    (설정 시 나타나는 16자리 앱 비밀번호를 계정 패스워드 대신 사용)
	 */
	passwordAuthentication = new PasswordAuthentication("kpg8226", "gxhjzdverytkbjie");
	}
